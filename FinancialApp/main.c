...
void test_get_day(){
    assert(get_day("23.03.2156")==23);
    assert(get_day("01.11.3467")==1);
    assert(get_day("92.23.2366")==92);
}

void test_get_mon(){
    assert(get_mon("23.03.2156")==3);
    assert(get_mon("01.11.3467")==11);
    assert(get_mon("92.23.2366")==23);
}

void test_get_year(){
    assert(get_year("23.03.2156")==2156);
    assert(get_year("01.11.3467")==3467);
    assert(get_year("92.23.2366")==2366);
}
...
float set_amount(){
    ...
    **/
    ...
    while(scanf("%f",&s)!=1) {
        printf("\n  [!] Input must be a real number! Try again: ");
        while (getchar() != '\n');
    }
    return s;
}
...
int get_day(char s[]){ ///returns the day as int
    return (s[0]-'0')*10+(s[1]-'0');
}
int get_mon(char s[]){ ///returns the month as int
    return (s[3]-'0')*10+(s[4]-'0');
}
int get_year(char s[]){ ///returns the year as int
            ...
            if((y1<y2)||(m1<m2&&y1==y2)||(d1<d2&&m1==m2&&y1==y2)){
                ...
                for(int i=0;i<n;i++)
                    if((y1<data[i].year&&y2>data[i].year)||(y1<=data[i].year&&y2>=data[i].year&&m1<data[i].month&&m2>data[i].month)||(y1<=data[i].year&&y2>=data[i].year&&m1<=data[i].month&&m2>=data[i].month&&d1<=data[i].day&&d2>=data[i].day)){
                        t++;
                        if(ok==0) printf("\n    Transactions list in given period:\n\n");
                        printf(">> %d.  Date: %d.%d.%d,   Amount: %.2f$,   Type: %s     -Description: %s",t, data[i].day, data[i].month, data[i].year, data[i].amount, data[i].type, data[i].description);
                        ok=1;
                    }
...
void Store(){
    ...
    **/
    if(n==0){
        printf("\n    [!] You have no transactions to store from this session!\n");
    }
    else{
...
void Load(){
    ...
    **/
    ...
    fclose(read);
    if(n==0) printf("\n    [!] You have no transactions to load from previous sessions!\n");
...
int read_valid(){
    /**
    stays in a while loop until user inserts a valid input(an int)
    return: x=int number
    **/
    int x;
    while(scanf("%d",&x)!=1) {
        printf("\n  [!] Input must be a integer number! Try again: ");
        while (getchar() != '\n');
    }
    return x;
}
...
int command_read(){
    /**
    reads the command
    return: a valid command or -1 in the command is not valid
    **/
    int x;
    printf("\n      ---> Insert a code to make an action: ");
    x=read_valid();
    if(x>6||x<0){
        printf("\n  [!] Input must be a number between 0 and 6!");
        return -1;
    }
    return x;
}
